...
namespace CodeImp.DoomBuilder.Plugins.NodesViewer
{
    ...
    public class NodesViewerMode : ClassicMode
    {
        ...
        public NodesForm Form { get; private set; }
        ...
        private bool LoadClassicStructures()
        {
            ...
            Vertices = new Vector2D[numverts];
            for (int i = 0; i < Vertices.Length; i++)
            {
                Vertices[i].x = vertsreader.ReadInt16();
                Vertices[i].y = vertsreader.ReadInt16();
            }
            vertsreader.Close();
            ...
            Subsectors = new Subsector[numssec];
            for (int i = 0; i < Subsectors.Length; i++)
            {
                Subsectors[i].numsegs = ssecreader.ReadUInt16(); //TECH: these are short in Doom, ushort in ZDoom/PRBoom+
                Subsectors[i].firstseg = ssecreader.ReadUInt16();
            }
            ssecreader.Close();
            ...
            for (int i = 0; i < Subsectors.Length; i++)
            {
                int lastseg = Subsectors[i].firstseg + Subsectors[i].numsegs - 1;
                for (int sg = Subsectors[i].firstseg; sg <= lastseg; sg++)
                {
                    Segs[sg].ssector = i;
                }
            }
        ...
        private bool LoadZNodes(string lumpName)
        {
            ...
            using (BinaryReader reader = new BinaryReader(stream))
            {
                ...
                // Add map vertices
                Vertices = new Vector2D[vertscount + newvertscount];
                int counter = 0;
                foreach (Vertex v in General.Map.Map.Vertices) Vertices[counter++] = v.Position;

                // Read extra vertices
                for (int i = counter; i < counter + newvertscount; i++)
                {
                    Vertices[i].x = reader.ReadInt32() / 65536.0f;
                    Vertices[i].y = reader.ReadInt32() / 65536.0f;
                }

                // Read subsectors
                uint sseccount = reader.ReadUInt32();
                Subsectors = new Subsector[sseccount];

                int firstseg = 0;
                for (int i = 0; i < Subsectors.Length; i++)
                {
                    Subsectors[i].numsegs = (int)reader.ReadUInt32();
                    Subsectors[i].firstseg = firstseg;
                    firstseg += Subsectors[i].numsegs;
                }
                ...
                if (nodesformat == "XGLN" || nodesformat == "XGL2" || nodesformat == "XGL3" || nodesformat == "ZGLN" || nodesformat == "ZGL2" || nodesformat == "ZGL3")
                {
                    ...
                    foreach (Subsector ss in Subsectors)
                    {
                        ...
                        {
                            Segs[i].angle = Vector2D.GetAngle(Vertices[Segs[i].endvertex], Vertices[Segs[i].startvertex]);
        ...
        private void BuildSubsectorPoly(int ss, IEnumerable<Split> nodesplits)
        {
            ...
            for (int i = 0; i < Subsectors[ss].numsegs; i++)
            {
                Split s;
                Seg sg = Segs[Subsectors[ss].firstseg + i];

                //mxd. Sanity check, because some segs in Doom maps refer to non-existing verts.  
                if (sg.startvertex > Vertices.Length - 1 || sg.endvertex > Vertices.Length - 1) continue;

                s.pos = Vertices[sg.startvertex];
                s.delta = Vertices[sg.endvertex] - Vertices[sg.startvertex];
                CropPolygon(poly, s);
            }
            ...
            Subsectors[ss].points = poly.ToArray();
        ...
        {
            if (Subsectors[index].points.Length == 0) return false; //mxd

            // Subsectors are convex, so we can simply test if the point is on the front side of all lines.
            Vector2D[] points = Subsectors[index].points;
        ...
        public override void OnRedrawDisplay()
        {
            ...
            if (renderer.StartPlotter(true))
            {
                if (Form.SelectedTab == 0)
                {
                    ...
                    for (int si = 0; si < Subsectors.Length; si++)
                    {
                        Subsector s = Subsectors[si];
                        PlotSubsectorLines(s.points, PixelColor.FromColor(Color.Gray));
                    }

                    if (mouseinssector > -1)
                    {
                        PlotSubsectorLines(Subsectors[mouseinssector].points, General.Colors.Highlight);
                    }

                    // Draw additional vertices
                    if (Form.ShowSegsVertices)
                    {
                        for (int i = General.Map.Map.Vertices.Count; i < Vertices.Length; i++)
                            renderer.PlotVertexAt(Vertices[i], ColorCollection.VERTICES);
                    }
                }
                ...
                if (Form.SelectedTab == 2)
                {
                    ...
                    if ((Form.ViewSubsectorIndex >= 0) && (Form.ViewSubsectorIndex < Subsectors.Length))
                    {
                        Subsector s = Subsectors[Form.ViewSubsectorIndex];
                    ...
                    if (Form.ViewSegIndex > -1)
                    {
                        ...
                        renderer.PlotLine(Vertices[sg.startvertex], Vertices[sg.endvertex], General.Colors.Selection);
                    }
                }
                ...
                switch (Form.SelectedTab)
                {
                    case 0:
                        if (mouseinssector > -1)
                        {
                            ...
                            DrawSubsectorArea(Subsectors[mouseinssector].vertices, General.Colors.Highlight);
                        }
                        else
                        {
                    ...
                    case 2:
                        if ((Form.ViewSubsectorIndex >= 0) && (Form.ViewSubsectorIndex < Subsectors.Length))
                        {
                            ...
                            {
                                Seg sg = Segs[Form.ViewSegIndex];
                                renderer.RenderLine(Vertices[sg.startvertex], Vertices[sg.endvertex], 1f, General.Colors.Selection, true);
                            }
                        }
                        break;
                }
