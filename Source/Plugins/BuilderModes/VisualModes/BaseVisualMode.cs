...
namespace CodeImp.DoomBuilder.BuilderModes
{
    ...
    public class BaseVisualMode : VisualMode
    {
        ...
        private struct SidedefAlignJob
        {
            ...
            public int ceilingHeight { get; private set; } //mxd
        ...
        public bool UseSelectionFromClassicMode { get; private set; } //mxd
        ...
        private void PostAction()
        {
            ...
            // Only clear the selection if anything is selected, since it can be very time consuming on huge maps
            if (IsSingleSelection && selectedobjects.Count > 0) ClearSelection();
        ...
        private IVisualEventReceiver GetTargetEventReceiver(bool targetonly)
        {
            if (target.picked != null)
            {
                if (IsSingleSelection || target.picked.Selected || targetonly || target.picked is VisualSlope)
                {
        ...
        private void AutoAlignTexturesUDMF(BaseVisualGeometrySidedef start, ImageData texture, bool alignx, bool aligny, bool resetsidemarks, bool checkselectedsidedefparts)
        {
            ...
            if (checkselectedsidedefparts && !IsSingleSelection)
            {
                foreach (IVisualEventReceiver i in selectedobjects)
                {
            ...
            while (todo.Count > 0)
            {
                ...
                if (VisualSectorExists(j.sidedef.Sector))
                {
                    ...
                    matchtop = !j.sidedef.Marked && (!IsSingleSelection || texturehashes.Contains(j.sidedef.LongHighTexture)) && parts.upper != null && parts.upper.Triangles > 0;
                    matchbottom = !j.sidedef.Marked && (!IsSingleSelection || texturehashes.Contains(j.sidedef.LongLowTexture)) && parts.lower != null && parts.lower.Triangles > 0;
                    matchmid = (!IsSingleSelection || texturehashes.Contains(j.controlSide.LongMiddleTexture))
        ...
        private void AddSidedefsForAlignment(Stack<SidedefAlignJob> stack, Vertex v, bool forward, double offsetx, double scaleY, HashSet<long> texturelongnames, bool udmf)
        {
            foreach (Linedef ld in v.Linedefs)
            {
                ...
                if ((ld.Start == v) && (side1 != null) && !side1.Marked)
                {
                    ...
                    {
                        if (!s.Marked && (!IsSingleSelection || BuilderModesTools.SidedefTextureMatch(this, s, texturelongnames)))
                        {
