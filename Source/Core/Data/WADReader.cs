...
namespace CodeImp.DoomBuilder.Data
{
    internal sealed class WADReader : DataReader
    {
        ...
        internal WAD WadFile { get; private set; } //mxd
        ...
        public WADReader(DataLocation dl, GameConfiguration config, bool asreadonly) : base(dl, asreadonly)
        {
            ...
            // Initialize
            WadFile = new WAD(location.location, asreadonly);
        ...
        private void Initialize(GameConfiguration config)
        {
            ...
            isreadonly |= WadFile.IsReadOnly; // Just in case...
            ...
            if (flatranges.Count > 0)
            {
                ...
                if (flatranges[flatranges.Count - 1].end < WadFile.Lumps.Count - 1)
                {
                    LumpRange range = new LumpRange { start = flatranges[flatranges.Count - 1].end + 1, end = WadFile.Lumps.Count - 1 };
                    invertedflatranges.Add(range);
                }
            }
            else // No flat ranges? Make one giant range then... 
            {
                LumpRange range = new LumpRange { start = 0, end = WadFile.Lumps.Count - 1 };
                invertedflatranges.Add(range);
            }
        }
        ...
        {
            base.Resume();
            WadFile = new WAD(location.location, IsReadOnly);
        }
        ...
        private void FindRanges(GameConfiguration config, List<LumpRange> ranges, IDictionary rangeinfos, string rangename, string elementname)
        {
            ...
            foreach (DictionaryEntry r in rangeinfos)
            {
                ...
                {
                    // Find ranges
                    int startindex = WadFile.FindLumpIndex(rangestart);
                    while (startindex > -1)
                    {
                        LumpRange range = new LumpRange { start = startindex, end = WadFile.FindLumpIndex(rangeend, startindex) };
                        if (range.end > -1)
                        {
                            if (!successfulrangestarts.ContainsKey(startindex)) successfulrangestarts.Add(startindex, false); //mxd
                            ranges.Add(range);
                            startindex = WadFile.FindLumpIndex(rangestart, range.end);
                        }
                        else
                        {
                            //mxd
            ...
            if (!WadFile.IsOfficialIWAD)
            {
                ...
                foreach (LumpRange range in ranges)
                {
                    ...
                    {
                        if (names.Contains(WadFile.Lumps[i].Name))
                        {
                            if (!Silent) General.ErrorLogger.Add(ErrorType.Warning, elementname + " \"" + WadFile.Lumps[i].Name + "\", index " + i + " is double defined in resource \"" + location.GetDisplayName() + "\".");
                        }
                        else
                            names.Add(WadFile.Lumps[i].Name);
                    }
                }
            }
        }
        ...
        public override Playpal LoadPalette()
        {
            ...
            Lump lump = WadFile.FindLump("PLAYPAL");
            if (lump != null)
            {
        ...
        public override ColorMap LoadMainColorMap(Playpal palette)
        {
            ...
            Lump lump = WadFile.FindLump("COLORMAP");
            if (lump != null)
            {
                using (Stream s = lump.GetSafeStream())
        ...
        private void LoadColormapsRange(string startlump, string endlump, ref List<ImageData> images)
        {
            ...
            int startindex = WadFile.FindLumpIndex(startlump);
            while (startindex > -1)
            {
                // Find end index
                int endindex = WadFile.FindLumpIndex(endlump, startindex + 1);
                if (endindex > -1)
                {
                    ...
                    for (int i = startindex + 1; i < endindex; i++)
                    {
                        ...
                        if (WadFile.Lumps[i].Length > 0)
                        {
                            // Make the image object
                            ColormapImage image = new ColormapImage(WadFile.Lumps[i].Name);
                ...
                // Find the next start
                startindex = WadFile.FindLumpIndex(startlump, startindex + 1);
            }
        }
        ...
        public override Stream GetColormapData(string pname)
        {
            ...
            if (strictpatches)
            {
                ...
                {
                    Lump lump = WadFile.FindLump(pname, range.start, range.end);
            ...
            else
            {
                ...
                Lump lump = WadFile.FindLump(pname);
                if (lump != null) return lump.GetSafeStream();
            }
        ...
        public override IEnumerable<ImageData> LoadTextures(PatchNames pnames, Dictionary<string, TexturesParser> cachedparsers)
        {
            ...
            Lump lump = WadFile.FindLump("TEXTURE1");
            if (lump != null)
            {
            ...
            lump = WadFile.FindLump("TEXTURE2");
            ...
            foreach (LumpRange range in textureranges)
            {
                ...
                for (int i = range.start + 1; i < range.end; i++)
                {
                    ...
                    {
                        // Make the image
                        SimpleTextureImage image = new SimpleTextureImage(WadFile.Lumps[i].Name, WadFile.Lumps[i].Name, defaultscale, defaultscale);
                    ...
                    else
                    {
                        ...
                        if (!Silent) General.ErrorLogger.Add(ErrorType.Error, "Can't load texture \"" + WadFile.Lumps[i].Name + "\" from \"" + location.GetDisplayName() + "\" because it doesn't contain any data.");
                    }
                }
            }

            // Load TEXTURES lump file
            int lumpindex = WadFile.FindLumpIndex("TEXTURES");
            while (lumpindex > -1)
            {
                ...
                {
                    Stream filedata = WadFile.Lumps[lumpindex].GetSafeStream();
                ...
                // Find next
                lumpindex = WadFile.FindLumpIndex("TEXTURES", lumpindex + 1);
            }
        ...
        public override IEnumerable<HiResImage> LoadHiResTextures()
        {
            ...
            foreach (LumpRange range in hiresranges)
            {
                ...
                for (int i = range.start + 1; i < range.end; i++)
                {
                    ...
                    {
                        // Add image to collection
                        images.Add(new HiResImage(WadFile.Lumps[i].Name));
                    }
                    else
                    {
                        // Can't load image without size
                        if (!Silent) General.ErrorLogger.Add(ErrorType.Error, "Can't load HiRes texture \"" + WadFile.Lumps[i].Name + "\" from \"" + location.GetDisplayName() + "\" because it doesn't contain any data.");
                    }
                }
            }
        ...
        public override PatchNames LoadPatchNames()
        {
            ...
            Lump lump = WadFile.FindLump("PNAMES");
            if (lump != null)
            {
                using (Stream s = lump.GetSafeStream())
        ...
        public override Stream GetPatchData(string pname, bool longname, ref string patchlocation)
        {
            ...
            {
                lump = WadFile.FindLump(pname, range.start, range.end);
                if (lump != null) return lump.GetSafeStream();
            }
        ...
        public override Stream GetHiResTextureData(string name, ref string hireslocation)
        {
            ...
            foreach (LumpRange range in hiresranges)
            {
                Lump lump = WadFile.FindLump(name, range.start, range.end);
                if (lump != null)
                {
        ...
        public override IEnumerable<ImageData> LoadFlats(Dictionary<string, TexturesParser> cachedparsers)
        {
            ...
            foreach (LumpRange range in flatranges)
            {
                ...
                for (int i = range.start + 1; i < range.end; i++)
                {
                    ...
                    {
                        // Make the image object
                        FlatImage image = new FlatImage(WadFile.Lumps[i].Name);
        ...
        public override HashSet<string> GetSpriteNames()
        {
            ...
            foreach (LumpRange range in spriteranges)
            {
                for (int i = range.start; i < range.end + 1; i++)
                    if (IsValidSpriteName(WadFile.Lumps[i].Name)) result.Add(WadFile.Lumps[i].Name);
            }
        ...
        public override HashSet<string> GetVoxelNames()
        {
            ...
            foreach (LumpRange range in voxelranges)
            {
                ...
                for (int i = range.start + 1; i < range.end; i++)
                {
                    if (IsValidVoxelName(WadFile.Lumps[i].Name)) result.Add(WadFile.Lumps[i].Name);
                }
            }
        ...
        {
            List<TextResourceData> result = new List<TextResourceData>();
            int lumpindex = WadFile.FindLumpIndex(name);
            if (lumpindex != -1)
                result.Add(new TextResourceData(this, WadFile.Lumps[lumpindex].GetSafeStream(), name, lumpindex, true));
        ...
        {
            List<TextResourceData> result = new List<TextResourceData>();
            int lumpindex = WadFile.FindLastLumpIndex(name);
            if (lumpindex != -1)
                result.Add(new TextResourceData(this, WadFile.Lumps[lumpindex].GetSafeStream(), name, lumpindex, true));
        ...
        private List<TextResourceData> GetAllLumpsData(string name)
        {
            ...
            while (lumpindex > -1)
            {
                ...
                // Find next entry
                lumpindex = WadFile.FindLumpIndex(name, lumpindex + 1);
            }
        ...
        {
            if (lumpindex < 0 || WadFile.Lumps.Count <= lumpindex || WadFile.Lumps[lumpindex].Name != name.ToUpperInvariant())
                return null;

            Lump l = WadFile.Lumps[lumpindex];
            if (l != null)
                return (MemoryStream)l.GetSafeStream();
        ...
        internal override bool SaveFile(MemoryStream lumpdata, string lumpname)
        {
            ...
            int insertindex = WadFile.Lumps.Count;

            // Remove the lump if it already exists
            int li = WadFile.FindLumpIndex(lumpname);
            if (li > -1)
            {
                insertindex = li;
                WadFile.RemoveAt(li);
            ...
            // Insert new lump
            Lump l = WadFile.Insert(lumpname, insertindex, (int)lumpdata.Length);
            l.Stream.Seek(0, SeekOrigin.Begin);
            lumpdata.WriteTo(l.Stream);

            // Update WAD file
            WadFile.WriteHeaders();
        ...
        {
            if (isreadonly || lumpindex < 0 || WadFile.Lumps.Count <= lumpindex || WadFile.Lumps[lumpindex].Name != lumpname.ToUpperInvariant())
                return false;

            // Remove the lump
            WadFile.RemoveAt(lumpindex);

            // Insert new lump
            Lump l = WadFile.Insert(lumpname, lumpindex, (int)lumpdata.Length);
            l.Stream.Seek(0, SeekOrigin.Begin);
            lumpdata.WriteTo(l.Stream);
        ...
        {
            return WadFile.FindLumpIndex(lumpname) != -1;
        }

        internal override bool FileExists(string lumpname, int lumpindex)
        {
            return WadFile.FindLumpIndex(lumpname) == lumpindex;
        }
        ...
        {
            int index = WadFile.FindLumpIndex(lumpname);
            if (index == -1)
            {
        ...
        internal override bool CompileLump(string lumpname, int lumpindex, ScriptConfiguration scriptconfig, List<CompilerError> errors)
        {
            ...
            Lump lump = WadFile.FindLump(reallumpname, lumpindex, lumpindex);
            if (lump == null)
                throw new Exception("Unable to find lump \"" + reallumpname + "\" to compile in \"" + location.GetDisplayName() + "\".");

            // Determine source file
            string sourcefile = General.Map.FilePathName.Length > 0 ? General.Map.FilePathName : WadFile.Filename;
