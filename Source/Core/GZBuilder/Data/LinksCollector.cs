...
namespace CodeImp.DoomBuilder.GZBuilder.Data
{
    public static class LinksCollector
    {
        ...
        private class PathNode
        {
            ...
            public Dictionary<int, PathNode> NextNodes { get; } // Thing index, PathNode
            public Dictionary<int, PathNode> PreviousNodes { get; } // Thing index, PathNode
            ...
            public PathNode(Thing t, VisualBlockMap blockmap)
            {
                ...
                pos.z += GetCorrectHeight(t, blockmap, true);
                Position = pos;
                NextNodes = new Dictionary<int, PathNode>();
                PreviousNodes = new Dictionary<int, PathNode>();
            }
            ...
            internal void PropagateCurvedFlag()
            {
                ...
                foreach (PathNode node in PreviousNodes.Values)
                {
                    if (node.IsCurved) continue;
                    node.IsCurved = true;
                    node.PropagateCurvedFlag();
