...
namespace CodeImp.DoomBuilder.ZDoom
{
    public class AmbientSoundInfo
    {
        ...
        // Sound settings
        public AmbientType AmbientSoundType { get; private set; } = AmbientType.NONE;
        public AmbientMode AmbientSoundMode { get; private set; } = AmbientMode.NONE;
        ...
        internal bool Setup(SndInfoParser parser)
        {
            ...
            if (token == "point" || token == "surround" || token == "world")
            {
                ...
                switch (token)
                {
                    case "point": AmbientSoundType = AmbientType.POINT; break;
                    case "surround": AmbientSoundType = AmbientType.SURROUND; break;
                    case "world": AmbientSoundType = AmbientType.WORLD; break;
                }
            ...
            if (token == "continuous" || token == "random" || token == "periodic")
            {
                ...
                switch (token)
                {
                    case "continuous": AmbientSoundMode = AmbientMode.CONTINUOUS; break;
                    case "random": AmbientSoundMode = AmbientMode.RANDOM; break;
                    case "periodic": AmbientSoundMode = AmbientMode.PERIODIC; break;
                }
            }
            else
            {
            ...
            SoundDescription = index.ToString() + ": " + SoundName + " (" + AmbientSoundType.ToString().ToLowerInvariant();

            if (AmbientSoundType == AmbientType.POINT)
                SoundDescription += ", attenuation: " + attenuation;

            if (AmbientSoundMode == AmbientMode.CONTINUOUS)
                SoundDescription += ", " + AmbientSoundMode.ToString().ToLowerInvariant();
            else if (AmbientSoundMode == AmbientMode.RANDOM)
                SoundDescription += ", every " + minsecs + " to " + maxsecs + " seconds";
            else if (AmbientSoundMode == AmbientMode.PERIODIC)
        ...
        {
            // Store radii
            MinimumRadius = info.MinimumDistance / info.Attenuation;

            if (info.Rolloff == SoundInfo.RolloffType.LOG)
            {
                // Calculate from RolloffFactor
                MaximumRadius = info.MinimumDistance + (info.RolloffFactor * info.MinimumDistance);
            }
            else
            {
                MaximumRadius = info.MaximumDistance;
            }
            MaximumRadius /= info.Attenuation;
        }
