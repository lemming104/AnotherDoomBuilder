...
namespace CodeImp.DoomBuilder.ZDoom
{
    internal sealed class ModeldefStructure
    {
        ...
        public Dictionary<int, string> SkinNames { get; }
        public Dictionary<int, Dictionary<int, string>> SurfaceSkinNames { get; }
        public Dictionary<int, string> ModelNames { get; }
        ...
        public bool InheritActorPitch { get; private set; }
        public bool UseActorPitch { get; private set; }
        public bool UseActorRoll { get; private set; }
        public bool UseRotationCenter { get; private set; }
        public string DataPath { get; private set; } // biwa

        public Dictionary<string, HashSet<FrameStructure>> Frames { get; }
        ...
        internal ModeldefStructure()
        {
            ...
            SurfaceSkinNames = new Dictionary<int, Dictionary<int, string>>();
        }
        ...
        internal bool Parse(ModeldefParser parser)
        {
            ...
            while (!parsingfinished && parser.SkipWhitespace(true))
            {
                ...
                switch (token)
                {
                    case "path":
                        ...
                        DataPath = parser.StripTokenQuotes(parser.ReadToken(false)).Replace("\\", "/"); // Don't skip newline
                    ...
                    case "model":
                        ...
                        ModelNames[index] = Path.Combine(DataPath, token).Replace(Path.AltDirectorySeparatorChar, Path.DirectorySeparatorChar);
                        break;
                    ...
                    case "skin":
                        ...
                        SkinNames[skinindex] = Path.Combine(DataPath, token);
                        break;
                    ...
                    case "surfaceskin":
                        ...
                        // Store
                        if (!SurfaceSkinNames.ContainsKey(modelindex))
                            SurfaceSkinNames[modelindex] = new Dictionary<int, string>();

                        SurfaceSkinNames[modelindex][surfaceindex] = Path.Combine(DataPath, token).Replace(Path.AltDirectorySeparatorChar, Path.DirectorySeparatorChar);
                        break;
            ...
            foreach (int i in SurfaceSkinNames.Keys.OrderBy(k => k))
            {
                if (SurfaceSkinNames[i].Count > 0 && !ModelNames.ContainsKey(i))
                {
