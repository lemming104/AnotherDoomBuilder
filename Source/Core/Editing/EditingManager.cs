...
namespace CodeImp.DoomBuilder.Editing
{
    public sealed class EditingManager
    {
        ...
        internal List<EditModeInfo> ModesInfo { get; }
        public EditMode Mode { get; private set; }
        public EditMode NewMode { get; private set; }
        public Type PreviousMode { get; private set; }
        public Type PreviousStableMode { get; private set; }
        public Type PreviousClassicMode { get; private set; }
        ...
        {
            // Initialize
            ModesInfo = new List<EditModeInfo>();
        ...
        internal void Dispose()
        {
            ...
            if (!IsDisposed)
            {
                ...
                // Clean up
                foreach (EditModeInfo i in ModesInfo) i.Dispose(); //mxd
        ...
        {
            foreach (EditModeInfo emi in ModesInfo)
            {
        ...
        public bool DisengageVolatileMode()
        {
            ...
            if ((General.Map != null) && (Mode != null) && Mode.Attributes.Volatile && !disengaging)
            {
                ...
                disengaging = true;
                ChangeMode(PreviousStableMode.Name);
                return true;
            }
            else
            {
        ...
        public bool ChangeMode(EditMode nextmode)
        {
            ...
            if (Mode != null)
            {
                PreviousMode = Mode.GetType();
                if (!Mode.Attributes.Volatile)
                {
                    PreviousStableMode = PreviousMode;
                    if (Mode is ClassicMode) PreviousClassicMode = PreviousMode;
                }
            }
            else
            {
                PreviousMode = null;
                PreviousStableMode = null;
                PreviousClassicMode = null;
            }
            ...
            if (General.Plugins.ModeChanges(oldmode, NewMode))
            {
                ...
                General.WriteLogLine("Previous stable mode is " + TypeNameOrNull(PreviousStableMode) + ", previous classic mode is " + TypeNameOrNull(PreviousClassicMode));
                Mode = NewMode;
                disengaging = false;
