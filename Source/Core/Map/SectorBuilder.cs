...
namespace CodeImp.DoomBuilder.Map
{
    ...
    internal sealed class SectorBuilder
    {
        ...
        public List<LinedefSide> SectorEdges { get; }
        ...
        {
            SectorEdges = new List<LinedefSide>();
        ...
        internal bool TraceSector(Linedef line, bool front)
        {
            ...
            if (edges == null)
                return false;
            SectorEdges.AddRange(edges);
            return SectorEdges.Count > 0;
        }
        ...
        public bool IsValidSector()
        {
            ...
            Sector sector = SectorEdges[0].Front ?
                (SectorEdges[0].Line.Front != null ? SectorEdges[0].Line.Front.Sector : null) :
                (SectorEdges[0].Line.Back != null ? SectorEdges[0].Line.Back.Sector : null);

            // Sector is invalid if any edge has no current sector
            if (sector == null) return false;

            // Go through subsequent edges
            for (int a = 1; a < SectorEdges.Count; a++)
            {
                // Get edge sector
                Sector ssector = SectorEdges[a].Front ?
                    (SectorEdges[a].Line.Front != null ? SectorEdges[a].Line.Front.Sector : null) :
                    (SectorEdges[a].Line.Back != null ? SectorEdges[a].Line.Back.Sector : null);
